...
//		Main_Trigger = Main_Trigger_Left;
		...
		if(flag==0){
			...
			// For this eff need to set turn ep cut off
			EP_Cut_inv = -0.1;
			}
...
string_e_sample = Form("%4.1f<%s.cer.asum_c&&%s.cer.asum_c<%4.1f",GC_e_sample_min,Arm.Data(),Arm.Data() ,GC_e_sample_max);
			...
			// For this eff neew to turn the GC_cut off;
			GC_Cut_inc = -0.001;
			}
		}
	else{
...
*/	
	//Set the limits for this pid;
	
 	TString GECs = Good_Electron_Cuts(ARM);
...
*/
	TString tot_e_cut =  GECs+"&&"+e_sample_cut;
	TString tot_p_cut =  GECs+"&&"+pion_sample_cut;
