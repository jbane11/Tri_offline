...
///parseing the arguments out:
	TString filename="";	//Name of runlist
	int ow =1;		//First part of ow
	int debug=0;		//debug print statments off =0: on =1
	if(NumArgs == 0){ cout <<"Please enter the filename for the Runlist!" <<endl;
			  cin >> filename;
			}
	else if(NumArgs == 1){filename = Args[0];}
	else if(NumArgs == 2){if(stoi(Args[1])) ow = stoi(Args[1]);}
	else if(NumArgs == 3){if(stoi(Args[2])) ow = stoi(Args[2]);}
...
////////////////////////////////////////////////////
	...
	if(KinFile.run_file_status==0){cerr << "error @ Runlist"<<endl;return 0;}
///Call tri_tools function to get the runs
  const vector<Int_t> RunNoChain=gGet_RunNoChain(Run_String);
	...
	if(kfile== nullptr){cerr << " File not open " <<endl;
		...
		if(kfile== nullptr){cerr << " File not open, 2nd " <<endl; return 0;}		
	}
