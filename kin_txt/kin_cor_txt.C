...
void kin_cor_txt(TString filename =""){
  //Overwrite section
  int overwrite[20]={1, //Runnumber -0
		     0, //Charge -1
		     0, //DT -2
		     0, //Trigger eff -3
		     1, //PID eff -4
...
////////////////Addeding to file
	   ...
	   if(arm=="L"){for(int j=1;j<4;j++){fprintf(kfile,"%0.5f\t",DT[j]);}}
	     else{for(int j=5;j<8;j++){fprintf(kfile,"%0.5f\t",DT[j]);}}
	   fprintf(kfile,"%10.10f\t",Pid_eff);
	   fprintf(kfile,"\n");
