...
void getinfo_new(Int_t run=0){
  ...
  if(irun==-1) return;
  string temp="undecided";
  TChain* T = LoadRun(irun,"T",temp);
  // cout<<"temp="<<temp<<endl;
  if (irun<20000) char const *TStree="TSLeft";
  else char const *TStree="TSRight";
  TChain* TS = LoadRun(irun,TStree);
  ...
  cout<<"      modification ends here-shujie  "<<endl;
  return;
  //------------------------------------
