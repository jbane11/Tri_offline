...
ax = kins[0].plot(x='Xbj',y='Yield',yerr='Error',kind='scatter',label=kinnames[0])
for j in range(1,len(files),1):
    xkcd = mcd.XKCD_COLORS["xkcd:" + overlap[j]].upper()
    kins[j].plot(ax=ax,x='Xbj',y='Yield',yerr='Error',kind='scatter',color=xkcd, 
    marker=mark[j],label=kinnames[j])
       
ax2 = kins_short[0].plot(x='Xbj',y='Yield',yerr='Error',kind='scatter',label=kinnames[0])
for j in range(1,len(files),1):
    xkcd = mcd.XKCD_COLORS["xkcd:" + overlap[j]].upper()
    kins_short[j].plot(ax=ax2,x='Xbj',y='Yield',yerr='Error',kind='scatter',color=xkcd, 
    marker=mark[j],label=kinnames[j])
