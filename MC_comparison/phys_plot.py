...
for j in range(len(mcDF)):
    ...
    #else :
    mcDFs[j].plot(ax=ax,x='theta',y='yield',yerr=':error',kind='scatter',marker=markers[j], color='blue', label='MC run{}'.format(runs[j]))
    
    dataDFs[j].plot(ax=ax,x='theta',y='yield',yerr=':error',kind='scatter', marker=markers2[j], color='red', label='data run {}'.format(runs[j]))
    ratios[j].plot(ax=bx,x='theta' ,y='ratio', xerr='offset', kind='scatter', label='{}'.format(runs[j]),marker=markers2[j] )
    k=k+1
    print(j , runs[j])    
ax.set_yscale("log")
ax.grid(True)
